			...
			struct stats_record *stats_prev)
{
	...
	/* Assignment#2: Print other XDP actions stats  */
        for (int action_index = 0; action_index < XDP_ACTION_MAX; action_index++)
	{
		char *fmt = "%-12s %'11lld pkts (%'10.0f pps)"
			" %'11lld Kbytes (%'6.0f Mbits/s)"
			" period:%f\n";
		const char *action = action2str(XDP_PASS);
		rec  = &stats_rec->stats[action_index];
		prev = &stats_prev->stats[action_index];

		double period = calc_period(rec, prev);
		if (period == 0)
		       return;

		__u64 packets = rec->total.rx_packets - prev->total.rx_packets;
		double pps = packets / period;

                __u64 bytes = rec->total.rx_bytes - prev->total.rx_bytes;
                double bps = bytes / period;
...
void map_get_value_percpu_array(int fd, __u32 key, struct datarec *value)
{
	...
	struct datarec values[nr_cpus];
        ...
        if (bpf_map_lookup_elem(fd, &key, values) != 0) {
                fprintf(stderr, "ERR: bpr_map_lookup_elem failed on key: 0x%X\n", key);
                return;
        }
